#include <avr/io.h>
#include <stdlib.h>
#include <string.h>
#include <util/delay.h>

#include "LCDI2C.h"

#ifndef _BV
#define _BV(bit)        (1<<(bit))
#endif
#ifndef sbi
#define sbi(reg,bit)    reg |= (_BV(bit))
#endif

#ifndef cbi
#define cbi(reg,bit)    reg &= ~(_BV(bit))
#endif

#ifndef tbi
#define tbi(reg,bit)    reg ^= (_BV(bit))
#endif


void timerInit() {
  //Wybranie trybu pracy CTC z TOP OCR1A
  sbi(TCCR1B, WGM12);
  cbi(TCCR1B, WGM13);
  cbi(TCCR1A, WGM11);
  cbi(TCCR1A, WGM10);

  //Wybranie dzielnika czestotliwosci - częstotliwość Atmega328: 16 MHz
  sbi(TCCR1B, CS12);
  cbi(TCCR1B, CS11);
  cbi(TCCR1B, CS10);

  //Zapisanie do OCR1A wartosci odpowiadajacej 0,5s
  OCR1A = 62500;

  //Uruchomienie przerwania OCIE1A
  sbi(TIMSK1, OCIE1A);
}

int8_t liczba_h = 15;
int8_t liczba_m = 17;
int8_t liczba_s = 0;
int8_t godzina_h = 15;
int8_t godzina_m = 15;
int8_t godzina_s = 15;

int main() {
  timerInit();
  sbi(EICRA, ISC01);
  cbi(EICRA, ISC00);
  sbi(EIMSK, INT0);
  //0x27 - adres urzadzenia na I2C
  LCD_I2C Lcd = LCD_I2C(0x27);
  _delay_ms(100);
  Lcd.backLightOn();
  sei();


  uint8_t i = 0;
  char text[20];
  char text2[20];

  DDRB = 0x00;
  PORTB = 0x0F;

  DDRD = 0xFF;
  while (1) {


    if (bit_is_clear(PINB, 0)) {
      _delay_ms(20);
      if (bit_is_clear(PINB, 0)) {
        if (liczba_m < 60) {
          liczba_m++;
        }
      }
      while (bit_is_clear(PINB, 0));
    }


    if (bit_is_clear(PINB, 1)) {
      _delay_ms(20);
      if (bit_is_clear(PINB, 1)) {
        if (liczba_m > 0) {
          liczba_m--;
        }
      }
      while (bit_is_clear(PINB, 1));
    }

    if (bit_is_clear(PINB, 2)) {
      _delay_ms(20);
      if (bit_is_clear(PINB, 2)) {
        if (bit_is_set(PORTD, 0)) {
          cbi(PORTD, 0);
        }
      }
      while (bit_is_clear(PINB, 2));
    }


    if (godzina_s >= 60) {
      godzina_m++;
      godzina_s = 0;
    }
    if (godzina_m >= 60) {
      godzina_h++;
      godzina_m = 0;
    }
    if (godzina_h >= 24) {
      godzina_h = 0;
    }

    if (liczba_s >= 60) {
      liczba_m++;
      liczba_s = 0;
    }
    if (liczba_m >= 60) {
      liczba_h++;
      liczba_m = 0;
    }
    if (liczba_h >= 24) {
      liczba_h = 0;
    }
    
    Lcd.clear();
    sprintf(text, "%d:%d:%d", godzina_h, godzina_m, godzina_s);
    Lcd.writeText(text);
    Lcd.goTo(0, 1);
    sprintf(text2, "%d:%d:%d", liczba_h, liczba_m, liczba_s);
    Lcd.writeText(text2);

    if (godzina_s == liczba_s && godzina_m == liczba_m && godzina_h == liczba_h) {
      sbi(PORTD, 0);
    }

    _delay_ms(150);
  }
}


ISR(TIMER1_COMPA_vect) {
  godzina_s = godzina_s + 1;
}
